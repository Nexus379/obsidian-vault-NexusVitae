...
> > **Active SRS Due:** `$= dv.pages('#5note/3atomic/studycards AND !"zData" AND -"yArchive"').where(p => p.study_date && moment(String(p.study_date)).isSameOrBefore(moment(), 'day')).length` due today
...
if (pages.length > 0) {
    ...
    const rows = [];
    grouped.forEach(g => {
        let r1 = g.rows.filter(p => !p.study_rank || p.study_rank === 1).map(p => p.file.link).slice(0, 5).join("<br>") || "—";
        let r2 = g.rows.filter(p => p.study_rank === 2).map(p => p.file.link).slice(0, 5).join("<br>") || "—";
        let r3 = g.rows.filter(p => p.study_rank === 3).map(p => p.file.link).slice(0, 5).join("<br>") || "—";
        let r4 = g.rows.filter(p => p.study_rank === 4).map(p => p.file.link).slice(0, 5).join("<br>") || "—";
        let r5 = g.rows.filter(p => p.study_rank >= 5).map(p => p.file.link).slice(0, 5).join("<br>") || "—";
