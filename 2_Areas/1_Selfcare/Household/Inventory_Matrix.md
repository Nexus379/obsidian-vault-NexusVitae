...
> 2. Click on **`BUTTON[add-vestis-owner]`** inside the item and select the person.
> 3. The person will appear here in the matrix! Toggle `🔄` (Refill) to add it to the Shopping Hub!
> **`BUTTON[add-vestis-owner]`** ⬅️ **Click here to assign an item from the Vault to a person!**
```dataviewjs
const categories = [
    { title: "🧴 Selfcare & Personal", tag: "#6resource/entity/personal_care" },
    { title: "🏠 Household (General)", tag: "#6resource/entity/household", excludePath: "Cleaning_Supplies" },
    { title: "🧽 Cleaning Supplies", tag: "#6resource/entity/household", includePath: "Cleaning_Supplies" },
...
for (let cat of categories) {
    ...
    let pages = dv.pages(cat.tag);
    if (cat.excludePath) pages = pages.where(p => !p.file.path.includes(cat.excludePath));
    if (cat.includePath) pages = pages.where(p => p.file.path.includes(cat.includePath));
