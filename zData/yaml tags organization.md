---
arch:
  - "#0cal"
  - "#1stars"
  - "#2area"
  - "#3project"
  - "#4task"
  - "#5note"
  - "#6resou"
archtype:
  - "#0cal/1plm"
  - "#0cal/2ppm"
  - "#0cal/3pkm"
  - "#0cal/1review"
  - "#0cal/1review/weekly"
  - "#0cal/1review/monthly"
  - "#0cal/1review/quarterly"
  - "#0cal/1review/halfyear"
  - "#0cal/1review/yearly"
  - "#0cal/4projectlog"
  - "#0cal/5protocol"
  - "#1stars/1purpose"
  - "#1stars/2vision"
  - "#1stars/3goals"
  - "#2area/1selfcare"
  - "#2area/3mind"
  - "#2area/2relationship"
  - "#2area/4organize"
  - "#2area/5creativity"
  - "#2area/6activity"
  - "#2area/7entertain"
  - "#3project/prodo"
  - "#3project/progo"
  - "#3project/prostudy"
  - "#3project/promeet"
  - "#3project/probuy"
  - "#3project/propay"
  - "#3project/procook"
  - "#3project/procraft"
  - "#3project/proget"
  - "#4task/todo"
  - "#4task/togo"
  - "#4task/tostudy"
  - "#4task/tostudy/spaced"
  - "#4task/tostudy/test"
  - "#4task/tostudy/exam"
  - "#4task/tostudy/class"
  - "#4task/tostudy/general"
  - "#4task/tomeet"
  - "#4task/tobuy"
  - "#4task/topay"
  - "#4task/tocook"
  - "#4task/tocraft"
  - "#4task/toget"
  - "#5note/1fleeting"
  - "#5note/2literature"
  - "#5note/3permanent"
  - "#5note/4atomic"
  - "#5note/4atomic/anki"
  - "#5note/4atomic/nutrition"
  - "#5note/5evergreen"
  - "#6resou/ai"
  - "#6resou/article"
  - "#6resou/boardgame"
  - "#6resou/book"
  - "#6resou/class"
  - "#6resou/course"
  - "#6resou/film"
  - "#6resou/game"
  - "#6resou/guide"
  - "#6resou/museum"
  - "#6resou/music"
  - "#6resou/paper"
  - "#6resou/recipe"
  - "#6resou/reference"
  - "#6resou/serie"
  - "#6resou/video"
inbox: false
axis:
  - "PLM"
  - "PPM"
  - "PKM"
cal0:
stars1:
area2:
project3:
task4:
note5:
resource6:
status:
  - "0recurring"
  - "0start"
  - "1active"
  - "2passive"
  - "3idea"
  - "done"
  - "canceled"
  - "review"
  - "archive"
  - "bin"
explore_lvl:
  - "0blueprint"
  - "1research"
  - "3investing"
  - "4polish"
  - "5finish"
priority:
cover:
url:
plattform:
  - Netflix
  - Crunchyroll
  - Disney+
  - Paramount
  - YouTube
  - Spotify
  - Google-AI
  - Mistral-AI
  - OpenAI-AI
  - Anthropic-AI
  - Perplexity-AI
  - Local
author:
artist:
publisher:
pub-date:
season-max:
season-now:
episode-now:
genre:
  - Self-Dev
  - Philosophy
  - Spirituality
  - Biography
  - Adventure
  - Fantasy
  - Sci-Fi
  - Thriller
  - Drama
  - Romance
  - Dark Romance
  - Horror
  - Classic
  - Manga
  - Poetry
  - Science
  - History
  - Business
  - Cooking
  - Guide
  - Other
parent:
sibling:
child:
summary:
review:
science:
  - "#sci/NaturalSci"
  - "#sci/SocialSci"
  - "#sci/Humanities"
  - "#sci/Engineering"
  - "#sci/Med-and-HealthSci"
  - "#sci/HealthSci"
  - "#sci/BusinessAdmin"
  - "#sci/Law"
  - "#sci/Arts"
  - "#sci/Agri-and-ForestSci"
  - "#sci/SportSci"
  - "#sci/Communic-and-MediaStudies"
  - "#sci/InformationSci"
  - "#sci/DataSci"
  - "#sci/EnvironmentalSci"
  - "#sci/Education"
  - "#sci/TheoANDPhilo-of-Religion"
  - "#sci/Lib-and-InfoSci"
  - "#sci/PlanningStudies"
  - "#sci/Spirituality"
  - "#sci/Metaphysics"
  - "#sci/Philosophy"
  - "#sci/Linguistics"
  - "#sci/TraditionalMedicine"
  - "#sci/AppliedSci"
discipline:
  - "#disc/GeneralStudies"
  - "#disc/NatureStudies"
  - "#disc/Arithmetic"
  - "#disc/Writing"
  - "#disc/LocalHistory"
  - "#disc/ArtClass"
  - "#disc/PhysicalEducation"
  - "#disc/GermanLanguage"
  - "#disc/EnglishLanguage"
  - "#disc/Biology"
  - "#disc/Microbiology"
  - "#disc/Genetics"
  - "#disc/Chemistry"
  - "#disc/Biochemistry"
  - "#disc/Physics"
  - "#disc/QuantumMechanics"
  - "#disc/Mathematics"
  - "#disc/Statistics"
  - "#disc/Astronomy"
  - "#disc/Astrophysics"
  - "#disc/Geology"
  - "#disc/Ecology"
  - "#disc/Meteorology"
  - "#disc/Immunology"
  - "#disc/Botany"
  - "#disc/Zoology"
  - "#disc/ChakraStudies"
  - "#disc/Chakra/Root"
  - "#disc/Chakra/Sacral"
  - "#disc/Chakra/SolarPlexus"
  - "#disc/Chakra/Heart"
  - "#disc/Chakra/Throat"
  - "#disc/Chakra/ThirdEye"
  - "#disc/Chakra/Crown"
  - "#disc/Ayurveda"
  - "#disc/TCM"
  - "#disc/YogaPhilosophy"
  - "#disc/Hermetics"
  - "#disc/Alchemy"
  - "#disc/Astrology"
  - "#disc/Kabbalah"
  - "#disc/Theosophy"
  - "#disc/Meditation"
  - "#disc/Reiki"
  - "#disc/Shamanism"
  - "#disc/GermanStudies"
  - "#disc/EnglishStudies"
  - "#disc/LatinStudies"
  - "#disc/RomanceStudies"
  - "#disc/SanskritStudies"
  - "#disc/History"
  - "#disc/Philosophy"
  - "#disc/Ethics"
  - "#disc/Theology"
  - "#disc/Mythology"
  - "#disc/Archaeology"
  - "#disc/Linguistics"
  - "#disc/Logic"
  - "#disc/Literature"
  - "#disc/Psychology"
  - "#disc/Sociology"
  - "#disc/PoliticalScience"
  - "#disc/Economics"
  - "#disc/BusinessAdmin"
  - "#disc/Law"
  - "#disc/Pedagogy"
  - "#disc/Criminology"
  - "#disc/Anthropology"
  - "#disc/Medicine"
  - "#disc/Neuroscience"
  - "#disc/Anatomy"
  - "#disc/Nutrition"
  - "#disc/Pharmacy"
  - "#disc/SportsScience"
  - "#disc/Psychiatry"
  - "#disc/ComputerScience"
  - "#disc/AI"
  - "#disc/CyberSecurity"
  - "#disc/Engineering"
  - "#disc/Robotics"
  - "#disc/Architecture"
  - "#disc/DataScience"
  - "#disc/Craftsmanship"
  - "#disc/CulinaryArts"
  - "#disc/Agriculture"
  - "#disc/Biohacking"
  - "#disc/Finance"
  - "#disc/Sustainability"
  - "#disc/ArtHistory"
  - "#disc/Musicology"
  - "#disc/Design"
  - "#disc/Photography"
  - "#disc/FilmStudies"
  - "#disc/MediaStudies"
isbn:
original-title:
rating:
  - "1"
ranking:
chapter:
volume:
volume-max:
subject:
due:
do:
persona:
# --- PLM: Physical & Life Management ---
  - "guardian"
  - "warrior"
  - "nurturer"
  - "parent"
  - "child"
  - "sibling"
  - "partner"
  - "friend"
  - "lover"
  - "host"
  - "traveler"
  - "player"
  - "monk_nun"
# --- PKM: Personal Knowledge Management ---
  - "student"
  - "analyst"
  - "creator"
  - "teacher"
  - "author"
  - "speaker"
  - "explorer"
  - "alchemist"
  - "seeker"
  - "mystic"
  - "researcher"
  - "archivist"
  - "technician"
# --- PPM: Project & Productivity Management ---
  - "worker"
  - "trainer"
  - "strategist"
  - "organizer"
  - "healer"
  - "queen_king"
  - "diplomat"
  - "visionary"
  - "architect"
  - "entrepreneur"
  - "mentor"
  - "critic"
sleep:
mood:
energy:
journal-am:
journal-pm:
fitness-am:
fitness-pm:
selfcare-am:
selfcare-pm:
meal:
vita-snap:
focus: ""
aim1: ""
aim2: ""
aim3: ""
maintask1: ""
maintask2: ""
maintask3: ""
maintask4: ""
maintask5: ""
cal-date:
kcal:
protein-g:
carbs-g:
fat-g:
fiber-g:
vit-a:
vit-b1:
vit-b2:
vit-b3:
vit-b5:
vit-b6:
vit-b7:
vit-b9:
vit-b12:
vit-c:
vit-d:
vit-e:
vit-k:
magnesium-mg:
iron-mg:
zinc:
calcium:
potassium:
iodine:
omega3:
ingredients:
amounts:
mealtime:
  - "breakfast"    # Der energetische Start
  - "lunch"        # Die Mittagssynthese
  - "cena"         # Der Abendabschluss
  - "snack"        # Kleine Alchemie zwischendurch
  - "pre-workout"  # Fokus-Energie
  - "post-workout" # Regeneration
  - "fasting"      # Tee/Wasser-Phase
  - "bento"        # Protable Food
mealtype:
  - "main"         # Vollwertiges Gericht (Braten, Bowl, Pfanne)
  - "soup"         # Suppen & Eintöpfe
  - "salad"        # Rohe Alchemie
  - "juice"        # Kalte Extraktion (Säfte/Shots)
  - "smoothie"     # Gebundene Vitalstoffe
  - "sauce"        # Dips, Dressings & Saucen (Komponenten)
  - "bakery"       # Brot & Gebäck
  - "dessert"      # Süße Synthesen
  - "tea"          # Heißwasser-Extraktion
  - "ferment"      # Bakterielle Reifung

difficulty:
  - "1"
  - "2"
  - "3"
  - "4"
  - "5"
---

## File Types
### Templater Shortcuts
#0cal cal- = calendar
#1stars stars- = aims
#2area a- = areas
#3project p- = projects
#4task t- = tasks
#5note n- = notes
#6resou r- = resource

### 0 Calendar

#### PLM
#0cal/1plm

#### PPM
#0cal/2ppm

#### PKM
#0cal/3pkm

#### Review
#0cal/1review
#0cal/1review/weekly
#0cal/1review/monthly
#0cal/1review/quarterly
#0cal/1review/halfyear
#0cal/1review/yearly

#### Projectlog
#0cal/4projectlog

#### Protocol
#0cal/5protocol

### 1 Stars
#1stars/1purpose
#1stars/2vision
#1stars/3goals

### 2 Area
#2area/1selfcare
#2area/3mind
#2area/2relationship
#2area/4organize
#2area/5creativity
#2area/6activity
#2area/7entertain

### 3 Projects
#3project
#3project/prodo
#3project/progo
#3project/prostudy
#3project/promeet
#3project/probuy
#3project/propay
#3project/procook
#3project/procraft

### 4 Task
#4task
#4task/todo
#4task/togo
#4task/tostudy
#4task/tostudy/spaced
#4task/tostudy/test
#4task/tostudy/exam
#4task/tostudy/class
#4task/tostudy/general
#4task/tomeet
#4task/tobuy
#4task/topay
#4task/tocook
#4task/tocraft

### 5 Note
#5note/1fleeting
#5note/2literature
#5note/3permanent
#5note/4atomic
#5note/4atomic/anki
#5note/4atomic/nutrition
#5note/5evergreen

### 6 Source
#6resou
#6resou/ai
#6resou/article
#6resou/boardgame
#6resou/book
#6resou/class
#6resou/course
#6resou/film
#6resou/game
#6resou/guide
#6resou/museum
#6resou/music
#6resou/paper
#6resou/recipe
#6resou/reference
#6resou/serie
#6resou/video
#### Class
alle nach / sind ausgeschriebene Wörter: ENG = English zb41
#disc
#disc/ENG
#disc/LAT
#disc/GER
#disc/PSE
#disc/HIS
#disc/BIO
#disc/CHE
#disc/MAT
#disc/PHY
#disc/ART
#disc/PHI
#disc/MUS
#disc/THE

## Disciplines
#disc 
#disc/ENG
#disc/LAT
#disc/GER
#disc/POL

#disc/PSE
#disc/HIS
#disc/ECO
#disc/LAW
#disc/PHI

#disc/BIO
#disc/CHE
#disc/MAT
#disc/PHY
#disc/ComSci

#disc/ART 
#disc/MUS
#disc/THE 
### Persona
#persona
#persona/guardian
#persona/warrior
#persona/nurturer
#persona/parent
#persona/child
#persona/sibling
#persona/partner
#persona/friend
#persona/lover
#persona/host
#persona/traveler
#persona/worker
#persona/student
#persona/trainer
#persona/analyst
#persona/strategist
#persona/researcher
#persona/archivist
#persona/technician
#persona/player
#persona/alchemist
#persona/seeker
#persona/mystic
#persona/monk_nun
#persona/organizer
#persona/healer
#persona/queen_king
#persona/diplomat
#persona/creator
#persona/teacher
#persona/author
#persona/speaker
#persona/visionary
#persona/explorer
#persona/architect
#persona/entrepreneur
#persona/mentor
#persona/critic

## Science
### Scientiae
1. Natural Sciences - Naturwissenschaften
#science/NaturalSci 
2. Humanities - Soziale Wissenschaften
#science/Humanities 
3. Social Sciences - Sozialwissenschaften
#science/SocialSci 
4. Engineering - Ingenieurwissenschaften
#science/Engineering 
5. Medicine_and_Health Sciences - Medizin und Geseundheitswissenschaften
#science/Med-and-HealthSci 
6. Economics and Business Administration - Wirtschaftswissenschaften
#science/EcoANDBusiness-Admin 
7. Law - Rechtswissenschaften
#science/Law 
8. Arts - Künste
#science/Arts 
9. Agriculture and Forestry Sciences - Agrar- und Frostwissenschaft
#science/Agri-and-ForestSci 
### weniger gängige
10. Sport Sciences - Sportwissenschaften
#science/SportSci 
11. Communication and Media Studies - Kommunikatios-und Medienwissenschaft
#science/Communic-and-MediaStudies 
12. Information Sciences - Informatioswissenschaft
#science/InformationSci 
13. Environmental Sciences - Umweltwissenschaft
#science/EnvironmentalSci 
14. Education - Erziehungswissenschaft
#science/Education 
15. Theology and Philosophy of Religion - Theologie und Religionsphilosophie
#science/TheoANDPhilo-of-Religion 
16. Library and Information Science - Bibliotheks- und Informationswissenschaft
#science/Lib-and-InfoSci 
17. Planning Studies - Planungswissenschaft
#science/PlanningStudies 

