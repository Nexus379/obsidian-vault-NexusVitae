...
>
>> ```dataviewjs
>> const halfyears = dv.pages().where(p => String(p.archtype).includes("#0cal/6review/halfyear") && p.rev_end >= "<%- start %>" && p.rev_end <= "<%- end %>").sort(p => p.rev_end, "asc");
