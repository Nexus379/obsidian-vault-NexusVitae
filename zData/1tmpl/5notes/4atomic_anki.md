...
___
- **`TARGET DECK`**: Tells the plugin which deck in Anki the card should go to (e.g., your main deck "Nexus").
- **`FILE TAGS`**: Adds tags to the card in Anki so you can filter them better there.
- **`START` / `END`**: These are the boundaries. Everything in between is treated as one flashcard.
- **`{Basic}`**: This is the card type in Anki. "Basic" simply means: Question on the front, answer on the back.
- **`Front:` / `Back:`**: Here you write the text that should appear on the front or back of the card in Anki.
- **`ID: ...`**: This is the **fingerprint** of the card. If you change the text in Obsidian later, Anki recognizes it by this number: *"Ah, this is the card from the other day, I'll just update it"*, instead of creating a new copy.
