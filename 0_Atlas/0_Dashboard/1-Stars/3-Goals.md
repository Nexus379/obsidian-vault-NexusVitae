...
> >         };
> >         const activeGoals = dv.pages('#1stars/3goals AND !"zData" AND -"yArchive"').where(p => p.inbox !== true).where(p => !dv.array(p.status).some(s => String(s).toLowerCase().includes('done') || String(s).toLowerCase().includes('archive') || String(s).toLowerCase().includes('bin'))).length;
...
> > > FROM #1stars/3goals AND !"zData" AND -"yArchive"
> > > WHERE !contains(string(status), "done") AND !contains(string(status), "archive") AND !contains(string(status), "bin") AND inbox != true
...
> [!source] **Goals Library**
> ![[0_Atlas/Bases/1-Stars/Goals.base]]
