...
> > ### NEXUS NAVIGATOR
> > ```dataviewjs
> > const pages = dv.pages('("6_Resources/Music" OR #6resou/music) AND -"zData" AND -"yArchive"').where(p => p.inbox !== true);
...
> > const activeItems = dv.pages('("6_Resources/Music" OR #6resou/music) AND -"zData" AND -"yArchive"')
> >     .where(p => p.inbox !== true)
> >     .where(p => clean(p.status).includes("1active") || clean(p.status).includes("0start"));
> > 
> > let html = `<div style="display:flex; flex-direction:column; gap:4px;">`;
> > if (activeItems.length === 0) {
> >     html += `<div style="font-size:.8em; color:var(--text-faint);">No music currently in progress.</div>`;
> > } else {
> >     for (let it of activeItems.sort(it => it.file.mtime, 'desc').slice(0, 10)) {
> >         const meta = it.artist || it.creator || it.plattform || "";
