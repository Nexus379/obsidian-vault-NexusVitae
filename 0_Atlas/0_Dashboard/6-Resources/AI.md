...
> > ### NEXUS NAVIGATOR
> > ```dataviewjs
> > const pages = dv.pages('("6_Resources/AI" OR #6resou/ai) AND -"zData" AND -"yArchive"').where(p => p.inbox !== true);
...
> > const activeItems = dv.pages('("6_Resources/AI" OR #6resou/ai) AND -"zData" AND -"yArchive"')
...
> >         const meta = it.creator || it.author || it.artist || it.director || it.publisher || it.plattform || "";
