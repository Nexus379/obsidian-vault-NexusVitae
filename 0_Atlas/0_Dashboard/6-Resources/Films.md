...
> > ### NEXUS NAVIGATOR
> > ```dataviewjs
> > const pages = dv.pages('("6_Resources/Films" OR #6resource/film) AND -"zData" AND -"yArchive"').where(p => p.inbox !== true);
...
> > window.renderChart && window.renderChart({ type: 'doughnut', data: { labels: ['Films', 'Cover', 'Platform', 'Rated'], datasets: [{ data: values.some(v => v > 0) ? values : [1], backgroundColor: ['#fab387', '#cba6f7', '#89dceb', '#a6e3a1'], borderWidth: 0 }] }, options: { cutout: '80%', plugins: { legend: { position: 'bottom', labels: { color: textColor, font: { size: 9, weight: 'bold' }, usePointStyle: true } } } } }, this.container);
> > ```
> >
...
> > const activeItems = dv.pages('("6_Resources/Films" OR #6resource/film) AND -"zData" AND -"yArchive"')
> >     .where(p => p.inbox !== true)
> >     .where(p => clean(p.status).includes("1active") || clean(p.status).includes("0start"));
> > 
> > let html = `<div style="display:flex; flex-direction:column; gap:4px;">`;
> > if (activeItems.length === 0) {
> >     html += `<div style="font-size:.8em; color:var(--text-faint);">No films currently in progress.</div>`;
> > } else {
> >     for (let it of activeItems.sort(it => it.file.mtime, 'desc').slice(0, 10)) {
> >         const meta = it.director || it.creator || it.style || "";
> >         let progress = "";
> >         if (it.volume) progress = `Part ${it.volume}`;
> >         progress = String(progress).trim() || (clean(it.status).includes("0start") ? "Start" : "Active");
> > 
...
> [!source] **🎬 Films Library**
> ![[0_Atlas/Bases/6-Resources/Films.base]]
