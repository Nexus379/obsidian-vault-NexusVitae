...
> > ### NEXUS NAVIGATOR
> > ```dataviewjs
> > const pages = dv.pages('("6_Resources/Reference" OR #6resou/reference) AND -"zData" AND -"yArchive"').where(p => p.inbox !== true);
...
> > const activeItems = dv.pages('("6_Resources/Reference" OR #6resou/reference) AND -"zData" AND -"yArchive"')
