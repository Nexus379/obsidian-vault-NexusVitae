...
> > ### NEXUS NAVIGATOR
> > ```dataviewjs
> > const pages = dv.pages('("6_Resources/Boardgame" OR #6resou/boardgame) AND -"zData" AND -"yArchive"').where(p => p.inbox !== true);
...
> > const activeItems = dv.pages('("6_Resources/Boardgame" OR #6resou/boardgame) AND -"zData" AND -"yArchive"')
