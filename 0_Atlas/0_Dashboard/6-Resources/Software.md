...
> > ### NEXUS NAVIGATOR
> > ```dataviewjs
> > const pages = dv.pages('("6_Resources/Software_Maintenance" OR #6resource/software) AND -"zData" AND -"yArchive"').where(p => p.inbox !== true);
...
> > const activeItems = dv.pages('("6_Resources/Software_Maintenance" OR #6resource/software) AND -"zData" AND -"yArchive"')
> >     .where(p => p.inbox !== true)
> >     .where(p => clean(p.status).includes("1active") || clean(p.maintenance_status).includes("needs_update") || clean(p.maintenance_status).includes("broken"));
> > let html = `<div style="display:flex; flex-direction:column; gap:4px;">`;
> > if (activeItems.length === 0) {
> >     html += `<div style="font-size:.8em; color:var(--text-faint);">No software items need attention.</div>`;
> > } else {
> >     for (let it of activeItems.sort(it => it.file.mtime, 'desc').slice(0, 10)) {
> >         const meta = [it.software_type, it.version, it.plattform].filter(Boolean).join(" · ");
> >         const statusLabel = it.maintenance_status || "active";
