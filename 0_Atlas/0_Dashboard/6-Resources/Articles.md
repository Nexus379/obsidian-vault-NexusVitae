...
> > ### NEXUS NAVIGATOR
> > ```dataviewjs
> > const pages = dv.pages('("6_Resources/Articles" OR #6resou/article) AND -"zData" AND -"yArchive"').where(p => p.inbox !== true);
...
> > window.renderChart && window.renderChart({ type: 'doughnut', data: { labels: ['Articles', 'Cover', 'Science', 'Discipline'], datasets: [{ data: values.some(v => v > 0) ? values : [1], backgroundColor: ['#a6e3a1', '#cba6f7', '#89dceb', '#fab387'], borderWidth: 0 }] }, options: { cutout: '80%', plugins: { legend: { position: 'bottom', labels: { color: textColor, font: { size: 9, weight: 'bold' }, usePointStyle: true } } } } }, this.container);
> > ```
> >
...
> > const activeItems = dv.pages('("6_Resources/Articles" OR #6resou/article) AND -"zData" AND -"yArchive"')
...
> >         const meta = it.author || it.creator || it.publisher || it.plattform || "";
