...
> > ### NEXUS NAVIGATOR
> > ```dataviewjs
> > const pages = dv.pages('("6_Resources/Classes" OR #6resou/class) AND -"zData" AND -"yArchive"').where(p => p.inbox !== true);
...
> > const activeItems = dv.pages('("6_Resources/Classes" OR #6resou/class) AND -"zData" AND -"yArchive"')
...
> >         const meta = it.creator || it.subject || it.discipline || "";
