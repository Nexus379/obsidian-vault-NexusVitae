...
> >             return hay.includes('#2area/5expression') || hay.includes('2area/5expression') || hay.includes('2_areas/5_expression') || hay.includes('5_expression') || hay.includes('5expression');
> >         };
> >         const values = [
> >             dv.pages('#2area/5expression AND !"zData" AND -"yArchive"').where(p => p.inbox !== true).length,
...
> >         const chartData = { type: 'doughnut', data: { labels: hasData ? ['Expression', 'Projects', 'Tasks', 'Notes', 'Resources', 'Logs'] : ['Empty Orbit'], datasets: [{ data: hasData ? values : [1], backgroundColor: hasData ? ['#ffcc99', '#fab387', '#f38ba8', '#a6e3a1', '#cba6f7', '#89b4fa'] : ['var(--background-modifier-border)'], borderWidth: 0, hoverOffset: 12 }] }, options: { cutout: '80%', plugins: { legend: { position: 'bottom', labels: { color: textColor, font: { size: 12, weight: 'bold', family: 'serif' }, padding: 20, usePointStyle: true } } } } };
...
> > > FROM #2area/5expression AND !"zData" AND -"yArchive"
> > > WHERE inbox != true
...
> > > WHERE (contains(string(area2), "3_Mind") OR contains(string(area2), "5expression")) AND inbox != true
...
> > > WHERE (contains(string(area2), "3_Mind") OR contains(string(area2), "5expression") OR contains(string(archtype), "#2area/5expression") OR contains(string(file.outlinks), "3_Mind")) AND inbox != true
