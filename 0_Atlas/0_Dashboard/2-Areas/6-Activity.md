...
> >             return hay.includes('#2area/6activity') || hay.includes('2area/6activity') || hay.includes('2_areas/6_activity') || hay.includes('6_activity') || hay.includes('6activity');
> >         };
> >         const values = [
> >             dv.pages('#2area/6activity AND !"zData" AND -"yArchive"').where(p => p.inbox !== true).length,
...
> >         const chartData = { type: 'doughnut', data: { labels: hasData ? ['Activity', 'Projects', 'Tasks', 'Notes', 'Resources', 'Logs'] : ['Empty Orbit'], datasets: [{ data: hasData ? values : [1], backgroundColor: hasData ? ['#9999ff', '#fab387', '#f38ba8', '#a6e3a1', '#cba6f7', '#89b4fa'] : ['var(--background-modifier-border)'], borderWidth: 0, hoverOffset: 12 }] }, options: { cutout: '80%', plugins: { legend: { position: 'bottom', labels: { color: textColor, font: { size: 12, weight: 'bold', family: 'serif' }, padding: 20, usePointStyle: true } } } } };
...
> > > FROM #2area/6activity AND !"zData" AND -"yArchive"
> > > WHERE inbox != true
...
> > > WHERE (contains(string(area2), "6_Activity") OR contains(string(area2), "6activity")) AND inbox != true
...
> > > WHERE (contains(string(area2), "6_Activity") OR contains(string(area2), "6activity") OR contains(string(archtype), "#2area/6activity") OR contains(string(file.outlinks), "6_Activity")) AND inbox != true
...
> ![[0_Atlas/Bases/2-Areas/Activity.base]]
