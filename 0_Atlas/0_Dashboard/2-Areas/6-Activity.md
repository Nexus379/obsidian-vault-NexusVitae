...
> >         };
> >         const values = [
> >             dv.pages('#2area/6activity AND !"zData"').length,
...
> > > FROM #2area/6activity AND !"zData"
...
> > > WHERE contains(string(area2), "6_Activity") OR contains(string(area2), "6activity") OR contains(string(archtype), "#2area/6activity") OR contains(string(file.outlinks), "6_Activity")
