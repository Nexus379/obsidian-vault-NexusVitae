...
> >             return hay.includes('#2area/7entertain') || hay.includes('2area/7entertain') || hay.includes('2_areas/7_entertainment') || hay.includes('7_entertainment') || hay.includes('7entertain');
> >         };
> >         const values = [
> >             dv.pages('#2area/7entertain AND !"zData" AND -"yArchive"').where(p => p.inbox !== true).length,
...
> >         const chartData = { type: 'doughnut', data: { labels: hasData ? ['Entertainment', 'Projects', 'Tasks', 'Notes', 'Resources', 'Logs'] : ['Empty Orbit'], datasets: [{ data: hasData ? values : [1], backgroundColor: hasData ? ['#cc99ff', '#fab387', '#f38ba8', '#a6e3a1', '#cba6f7', '#89b4fa'] : ['var(--background-modifier-border)'], borderWidth: 0, hoverOffset: 12 }] }, options: { cutout: '80%', plugins: { legend: { position: 'bottom', labels: { color: textColor, font: { size: 12, weight: 'bold', family: 'serif' }, padding: 20, usePointStyle: true } } } } };
...
> > 
> > > [!collect] **🕹️ Entertainment Entries**
> > > ```dataview
> > > TABLE status, priority, area2, file.mtime AS updated
> > > FROM #2area/7entertain AND !"zData" AND -"yArchive"
> > > WHERE inbox != true
...
> > > WHERE (contains(string(area2), "7_Entertainment") OR contains(string(area2), "7entertain")) AND inbox != true
...
> > > WHERE (contains(string(area2), "7_Entertainment") OR contains(string(area2), "7entertain") OR contains(string(archtype), "#2area/7entertain") OR contains(string(file.outlinks), "7_Entertainment")) AND inbox != true
...
> [!source] **Entertainment Library**
> ![[0_Atlas/Bases/2-Areas/Entertainment.base]]
