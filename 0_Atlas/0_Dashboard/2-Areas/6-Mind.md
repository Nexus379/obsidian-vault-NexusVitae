...
> >             return hay.includes('#2area/6mind') || hay.includes('2area/3mind') || hay.includes('2_areas/3_mind') || hay.includes('3_mind') || hay.includes('3mind');
> >         };
> >         const values = [
> >             dv.pages('#2area/6mind AND !"zData" AND -"yArchive"').where(p => p.inbox !== true).length,
...
> > > WHERE (contains(string(area2), "3_Mind") OR contains(string(area2), "3mind") OR contains(string(archtype), "#2area/6mind") OR contains(string(file.outlinks), "3_Mind")) AND inbox != true
