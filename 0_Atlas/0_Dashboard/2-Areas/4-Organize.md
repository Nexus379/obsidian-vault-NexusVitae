...
> >             return hay.includes('#2area/4organize') || hay.includes('2area/4organize') || hay.includes('2_areas/4_organize') || hay.includes('4_organize') || hay.includes('4organize');
> >         };
> >         const values = [
> >             dv.pages('#2area/4organize AND !"zData" AND -"yArchive"').where(p => p.inbox !== true).length,
...
> >         const chartData = { type: 'doughnut', data: { labels: hasData ? ['Organize', 'Projects', 'Tasks', 'Notes', 'Resources', 'Logs'] : ['Empty Orbit'], datasets: [{ data: hasData ? values : [1], backgroundColor: hasData ? ['#b3ffb3', '#fab387', '#f38ba8', '#a6e3a1', '#cba6f7', '#89b4fa'] : ['var(--background-modifier-border)'], borderWidth: 0, hoverOffset: 12 }] }, options: { cutout: '80%', plugins: { legend: { position: 'bottom', labels: { color: textColor, font: { size: 12, weight: 'bold', family: 'serif' }, padding: 20, usePointStyle: true } } } } };
...
> > 
> > > [!organize] **🧩 Organize Entries**
> > > ```dataview
> > > TABLE status, priority, area2, file.mtime AS updated
> > > FROM #2area/4organize AND !"zData" AND -"yArchive"
> > > WHERE inbox != true
...
> > > WHERE (contains(string(area2), "4_Organize") OR contains(string(area2), "4organize")) AND inbox != true
...
> > > WHERE (contains(string(area2), "4_Organize") OR contains(string(area2), "4organize") OR contains(string(archtype), "#2area/4organize") OR contains(string(file.outlinks), "4_Organize")) AND inbox != true
...
> [!source] **Organize Library**
> ![[0_Atlas/Bases/2-Areas/Organize.base]]
