...
> >         const values = [dv.pages('"5_Notes/5_Evergreen" OR #5note/5evergreen').length, dv.pages('"3_Projects"').where(linked).length, dv.pages('"4_Tasks"').where(linked).length, dv.pages('"6_Resources"').where(linked).length, dv.pages('"0_Calendar"').where(linked).length];
...
> > > FROM "5_Notes/5_Evergreen" OR #5note/5evergreen AND !"zData"
...
> > > WHERE contains(string(note5), "5_Evergreen") OR contains(string(note5), "5evergreen") OR contains(string(file.outlinks), "5_Evergreen")
> > > SORT file.mtime DESC
> > > ```
> >
> > > [!project] **Linked Work**
...
> > > WHERE contains(string(note5), "5_Evergreen") OR contains(string(note5), "5evergreen") OR contains(string(archtype), "#5note/5evergreen") OR contains(string(file.outlinks), "5_Evergreen")
